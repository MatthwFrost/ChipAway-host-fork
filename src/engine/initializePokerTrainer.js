...
function sampleFromRange(pl,blocked){
  ...
  for(let t=0;t<26;t++){
    ...
    if(e&&!blocked[cardStr(e.h[0])]&&!blocked[cardStr(e.h[1])]) return e.h;
  }
  for(let t=0;t<40;t++){
...
function calcEquity(hole,opponents,trials,useRanges){
  ...
  if(nOpp<=0) return {win:100,tie:0,lose:0};
  const baseBlocked={};
  ...
  for(let t=0;t<trials;t++){
    ...
    for(let o=0;o<nOpp;o++){
      let h=null;
      if(useRanges) h=sampleFromRange(opponents[o],blocked);
      if(!h){
    ...
    for(let o=0;o<oppHands.length;o++){
      ...
      if(bo===null||cmpScore(s,bo)>0)bo=s;
    }
    const c=cmpScore(mine,bo);
    if(c>0)win++;else if(c===0)tie++;
    done++;
  }
  if(!done) return {win:0,tie:0,lose:100};
  return {win:100*win/done,tie:100*tie/done,lose:100*(done-win-tie)/done};
}
...
function initPlayers(){
  players=[{id:0,name:'You',profile:null,stack:START_STACK,hole:[],folded:false,allIn:false,bet:0,committed:0,acted:false,isHero:true,badge:'',badgeCls:'',raises:0,rLo:0,rBluff:0}];
  for(let i=0;i<SEAT_NAMES.length;i++){
    players.push({id:i+1,name:SEAT_NAMES[i],profile:seatConfig[i],stack:START_STACK,hole:[],folded:false,allIn:false,bet:0,committed:0,acted:false,isHero:false,badge:'',badgeCls:'',raises:0,rLo:0,rBluff:0});
  }
...
function showEquity(){
  ...
  $('eqSub').innerHTML='vs their ranges &nbsp;·&nbsp; vs random <b>'+eqU.win.toFixed(1)+'%</b> ('+(diff>=0?'+':'')+diff.toFixed(1)+')'+
    (main?'<br>your cards block <b>'+blk.toFixed(1)+'%</b> of '+main.name+"'s value combos":'');
  ...
  const ranked=spot.options.slice().sort(function(a,b){return b.ev-a.ev;});
  const best=ranked[0];
  let rows='';
  ...
  if(best.label==='fold'){
    ...
    why='<b>'+best.label+'</b> wins most: they fold <b>'+(100*best.fold).toFixed(0)+'%</b> of the time, and when called you still hold <b>'+(eqR.win+eqR.tie/2).toFixed(0)+'%</b>.';
  } else if(best.label.indexOf('call')===0){
    why='Calling is best: you need <b>'+(100*toCall/(onTable+toCall)).toFixed(0)+'%</b> and hold <b>'+(eqR.win+eqR.tie/2).toFixed(0)+'%</b>, but raising folds out too much of what you beat.';
  } else {
...
function collectBets(){
  players.forEach(function(p){pot+=p.bet;p.bet=0;p.acted=false;p.raises=0;});
...
function startHand(){
  ...
  players.forEach(function(p){p.hole=[];p.folded=false;p.allIn=false;p.bet=0;p.committed=0;p.acted=false;p.raises=0;p.badge='';p.badgeCls='';p.rLo=0;p.rBluff=0;p.score=null;});
...
function heroRaise(){
  ...
  if(rs>0)minRaise=Math.max(minRaise,rs);
  currentBet=Math.max(currentBet,h.bet);
  h.acted=true;h.raises++;streetRaises++;lastAggressor=0;
  ...
  players.forEach(function(p){if(p!==h&&!p.folded&&!p.allIn)p.acted=false;});
...
function applyBotRaise(p,target,toCall,label){
  ...
  p.acted=true;p.raises++;streetRaises++;lastAggressor=players.indexOf(p);
  narrowAggro(p,put/Math.max(1,pb));
  setBadge(p,(p.allIn?'all in ':label+' ')+p.bet,'b-aggro');
  players.forEach(function(x){if(x!==p&&!x.folded&&!x.allIn)x.acted=false;});
...
function preflopAct(p){
  ...
  if(p.raises<prof.maxRaises&&streetRaises<4&&Math.random()<sigmoid((pct-tbThresh)/(w*0.8))){
...
function postflopAct(p){
  ...
  const canRaise=p.raises<prof.maxRaises&&streetRaises<4;
  const w=prof.mixR;
...
function heroEquityNow(){
  ...
  return clamp((eq.win+eq.tie/2)/100+positionalCredit(),0,1);
}
...
function snapshotSpot(){
  ...
  const e=heroEquityNow();
  const opts=[];
  opts.push({label:'fold',ev:0,amount:0});
  if(C>0) opts.push({label:'call '+C,ev:evOfCall(e,P,C),amount:C});
  else opts.push({label:'check',ev:0,amount:0});
  [[0.33,'\u2153 pot'],[0.5,'\u00bd pot'],[0.75,'\u00be pot'],[1,'pot'],[1.5,'1.5\u00d7 pot']].forEach(function(sz){
    ...
    opts.push({label:allIn?('all in '+target):((C>0?'raise to ':'bet ')+target+' ('+sz[1]+')'),ev:evOfBet(e,P,B,f),amount:B,target:target,fold:f,n:ref.length});
  });
  let best=opts[0];
  opts.forEach(function(o){ if(o.ev>best.ev) best=o; });
  return {street:street,equity:e,pot:P,toCall:C,options:opts,best:best};
}
...
function enableHeroControls(){
  ...
  sl.disabled=maxT<=minT;
  $('btnRaise').disabled=maxT<=minT;
